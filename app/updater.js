...
exports.init = (platform, ver) => {
  ...
  // There has never been a Linux update channel here - `platforms` has no entry
  // for it - and Electron's autoUpdater is macOS and Windows only, so there is
  // nothing to schedule.
  if (!platforms[platform]) {
    debug(`no update channel for platform ${platform}`)
    return
  }
...
var requestVersionInfo = (done) => {
  ...
  request(queryString, (err, response, body) => {
    ...
    if (status !== 200 && status !== 204) {
      debug(`update check got HTTP ${status}`)
    ...
    if (body) {
      try {
        ...
        debug(`update check response was not JSON: ${e.message}`)
        AppActions.setUpdateStatus(UpdateStatus.UPDATE_NOT_AVAILABLE)
        return
      }
    }
    ...
    process.emit(messages.UPDATE_META_DATA_RETRIEVED, metadata)
    done(null, metadata)
  })
}
...
var downloadHandler = (err, metadata) => {
  ...
  try {
    if (process.platform === 'win32' && !metadata) {
    ...
    // no update feed configured, which is the normal case for a source build
    debug(`autoUpdater unavailable: ${e.message}`)
    AppActions.setUpdateStatus(UpdateStatus.UPDATE_ERROR)
  }
}
...
exports.update = () => {
  // There is no update channel on this platform, so there is nothing to
  // install and quitAndInstall would only close the browser.
  if (!platforms[process.platform]) {
    debug('update requested but this platform has no update channel')
    AppActions.setUpdateStatus(UpdateStatus.UPDATE_NOT_AVAILABLE)
    return
  }
