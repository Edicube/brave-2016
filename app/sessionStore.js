...
module.exports.saveAppState = (payload) => {
  return new Promise((resolve, reject) => {
    ...
    const tempPath = storagePath + '.writing'
    fs.writeFile(tempPath, JSON.stringify(payload), (err) => {
      if (err) {
        reject(err)
        return
      }
      fs.rename(tempPath, storagePath, (renameErr) => {
        if (renameErr) {
          reject(renameErr)
        } else {
          resolve()
...
/**
 * Parses the session file with the usual prototype-pollution keys refused.
 * JSON.parse gives {"__proto__": ...} an own property of that name, and any
 * later merge that assigns through plain [[Set]] (Object.assign, spread)
 * turns it into a live prototype write. The session format has no legitimate
 * member by any of these names.
 * @param {string|Buffer} raw
 * @return {object}
 */
const parseSession = (raw) => JSON.parse(raw, (key, value) => {
