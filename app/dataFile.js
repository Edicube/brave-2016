...
const storagePath = (url) => {
  ...
  if (!base || base === '.' || base === '..') {
    throw new Error(`unusable data file url: ${url}`)
  }
  return path.join(app.getPath('userData'), base)
}
const downloadPath = (url) => `${storagePath(url)}.temp`

// A compromised or hostile mirror must not be able to fill the disk through
// the data file downloads.
...
function downloadSingleFile (resourceName, url, version, force, resolve, reject) {
  ...
  var req = request.get({
    ...
    if (declared > maxDownloadBytes) {
      response.destroy()
      reject('data file larger than the accepted maximum')
      return
    }
