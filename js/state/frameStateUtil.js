...
const Swatch = require('../lib/swatch.js')
...
/**
 * Extracts theme-color from a favicon.
 */
export function computeThemeColor (frameProps) {
  return new Promise((resolve, reject) => {
    ...
    // one of those doomed requests. Skip them instead of firing requests that
    // cannot succeed.
    ...
    function renderFromBlob (blob) {
      ...
      img.onload = () => {
        const color = Swatch.dominantColor(img)
        ...
        if (color) {
          ...
          reject(new Error('Could not determine a theme colour for ' + icon))
        }
      }
      img.onerror = () => {
        window.URL.revokeObjectURL(img.src)
